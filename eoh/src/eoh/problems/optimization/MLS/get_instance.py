...
def calc_fitness(peaks, misreports, weights, place_func, k, epsilon=0.01):
    ...
    # Max regret
    max_r = calc_max_regret(peaks, misreports, weights, place_func, k) if misreports is not None else 0.0
        ...
        for key, val in dataset.items():
            ...
            if 'peaks' in val and val['peaks'] is not None:
                ...
                if 'weights' in val and val['weights'] is not None:
                    ...
                    if len(wshape) == 1:
                        if wshape[0] != n:
                            print(f"Warning: mismatch in n for {key}: weights={wshape}, peaks n={n}")
                    elif len(wshape) == 2:
                        if wshape[1] != n:
                            print(f"Warning: mismatch in n for {key}: weights shape={wshape}, peaks n={n}")
